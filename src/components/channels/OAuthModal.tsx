...
export const OAuthModal: React.FC<OAuthModalProps> = ({
  ...
  const oauthConfig = PLATFORM_OAUTH_CONFIGS[selectedPlatform];
  ...
  useEffect(() => {
    ...
    } else if (platformConfig?.pages && platformConfig.pages.length > 0) {
      setCandidatePages(platformConfig.pages);
    } else {
      setCandidatePages([
        {
          id: `${selectedPlatform}-page-1`,
          name: `${user.organization || 'Meu Negócio'} Oficial`,
          username: `@${selectedPlatform}_${user.name.toLowerCase().replace(/\s+/g, '')}`,
          avatar: user.avatar,
          followers: 12500,
          type: selectedPlatform === 'facebook' || selectedPlatform === 'linkedin' ? 'Página Comercial' : 'Perfil Oficial',
          selected: true
        }
      ]);
    }
  ...
  const handleStartAuth = () => {
    ...
    // Dispara a popup oficial de autenticação da rede selecionada
    try {
      openPlatformOAuthWindow(selectedPlatform);
    } catch (err) {
      console.warn(`Popup blocked or not permitted for ${selectedPlatform}`, err);
    }
  ...
  const handleConfirmPages = () => {
    ...
    setTimeout(() => {
      ...
      const finalizedAccount: SocialAccount = {
        id: initialAccount?.id || `acc-${selectedPlatform}`,
        ...
        accountType: selectedPlatform === 'facebook' || selectedPlatform === 'linkedin' ? 'page' : 'business',
        workspaceName: user.workspaceName || 'Workspace Principal',
        authorizedPermissions: oauthConfig.scopes,
        subPages: candidatePages,
        metaAppId: isMeta ? META_APP_ID : undefined
      };

      if (initialAccount?.connected) {
        updateAccountSubPages(finalizedAccount.id, candidatePages);
      } else {
      ...
      <div className="relative w-full max-w-lg bg-slate-900 border border-slate-700/80 rounded-2xl sm:rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[92vh]">
        ...
        <div className="p-4 sm:p-5 bg-gradient-to-r from-slate-950 via-slate-900 to-slate-950 border-b border-slate-800 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            >
              {selectedPlatform === 'facebook' ? 'f' : (selectedPlatform === 'twitter' ? '𝕏' : platformInfo.name.charAt(0))}
            </div>
            <div>
              <div className="flex items-center gap-2">
                <span className="text-xs font-bold text-white tracking-wide">
                  {oauthConfig.displayName} • {oauthConfig.apiName}
                </span>
        ...
        <div className="px-5 py-2.5 bg-slate-950/50 border-b border-slate-800/80 flex items-center justify-between text-[11px] font-medium">
          ...
          <div className={`flex items-center gap-1.5 ${step >= 2 ? 'text-brand-400 font-bold' : 'text-slate-500'}`}>
            ...
            <span>{selectedPlatform === 'facebook' || selectedPlatform === 'linkedin' ? 'Seleção de Páginas' : 'Seleção de Perfis'}</span>
          </div>
          <div className="w-6 h-0.5 bg-slate-800" />
        ...
        <div className="p-5 sm:p-6 overflow-y-auto flex-1 space-y-5 text-slate-200 text-xs">
          ...
          {step === 1 && (
            <div className="space-y-4 animate-fade-in">
              ...
              {/* App ID / Client ID Dedicated Card */}
              <div 
                className="p-3.5 rounded-2xl border flex items-center justify-between gap-3 text-xs bg-slate-950/60"
              ...
              >
                  ...
                  <div>
                    <div className="flex items-center gap-1.5">
                      <span className="font-bold text-white text-xs">
                        {isMeta ? 'App ID Oficial Meta:' : 'Client ID Oficial:'}
                      </span>
                    ...
                    <div className="text-[10px] text-slate-400 mt-0.5">
                      {oauthConfig.apiName} • Protocolo OAuth 2.0
                    </div>
                  </div>
                </div>
                ...
                <button
                  type="button"
                  onClick={() => openPlatformOAuthWindow(selectedPlatform)}
                  className="px-2.5 py-1.5 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-300 hover:text-white text-[10px] font-bold flex items-center gap-1 border border-slate-700 transition-all shrink-0"
                >
                  <ExternalLink className="w-3 h-3" />
                  <span>Janela OAuth</span>
                </button>
              </div>

              {/* User Identity Box */}
              <div className="p-3.5 rounded-xl bg-slate-950/40 border border-slate-800 flex items-start gap-3">
                <div className="w-8 h-8 rounded-full bg-slate-800 border border-slate-700 flex items-center justify-center shrink-0 text-white font-bold text-xs">
                  {user.avatar ? (
                ...
                <div>
                  <div className="text-xs font-semibold text-white">
                    Autorizar em nome de {user.name}
                  </div>
              ...
              <div>
                <h4 className="text-sm font-bold text-white mb-1">
                  Autorizar o FlowContent a gerenciar publicações no {oauthConfig.displayName}
                </h4>
              ...
              <div className="p-4 rounded-xl bg-slate-950/70 border border-slate-800/90 space-y-2.5">
                <div className="text-[10px] uppercase font-bold tracking-wider text-slate-400 flex items-center gap-1.5">
                  ...
                  <span>Escopos oficiais solicitados ({oauthConfig.displayName}):</span>
                </div>
                ...
                <div className="pt-2 border-t border-slate-800/80 text-[11px] text-slate-400 flex items-center gap-1.5">
                  ...
                  <span>Token renovável com validade de 60 dias e criptografia de ponta a ponta.</span>
                </div>
              </div>
              ...
              >
                {authenticating ? (
                  <>
                    ...
                    <span>Conectando com o endpoint oficial do {oauthConfig.displayName}...</span>
                  </>
                ) : (
                  <>
                    <ExternalLink className="w-4 h-4" />
                    <span>Abrir Autenticação Oficial {oauthConfig.displayName} (OAuth)</span>
          ...
          {step === 2 && (
            <div className="space-y-4 animate-fade-in">
              <div className="flex items-center justify-between gap-2">
                <div>
                  <h4 className="text-sm font-bold text-white">
                    Contas e Páginas Detectadas no {oauthConfig.displayName}
                  </h4>
                  <p className="text-[11px] text-slate-400 mt-0.5">
                    Selecione quais perfis autorizar para disparos e agendamento em lote.
                  </p>
                </div>
              ...
              <div className="p-3 rounded-xl bg-slate-950/60 border border-slate-800/80 flex items-center gap-2.5 text-slate-400 text-[11px]">
                ...
                <span>
                  <strong>{selectedCount}</strong> conta(s) selecionada(s) para vinculação oficial no seu workspace.
                </span>
              </div>
              ...
              <div className="flex items-center gap-3 pt-2">
                ...
                >
                  {authenticating ? (
                    <>
                      <Loader2 className="w-4 h-4 animate-spin" />
                      <span>Gerando token OAuth 2.0 ativo...</span>
                    </>
                  ) : (
                    <>
            ...
            <div className="space-y-4 text-center py-4 animate-fade-in">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400 mt-1 max-w-sm mx-auto">
                  A conta do <strong>{oauthConfig.displayName}</strong> foi autorizada com token oficial ativo e já está sincronizada com seu workspace.
                </p>
              </div>

              <div className="p-4 rounded-xl bg-slate-950 border border-slate-800 text-left space-y-2 text-xs">
                <div className="flex items-center justify-between text-slate-300">
                  <span className="text-slate-400">Canal Conectado:</span>
                  <span className="font-bold text-white">{oauthConfig.displayName}</span>
                </div>
                <div className="flex items-center justify-between text-slate-300">
                  <span className="text-slate-400">Status do Token:</span>
                  <span className="font-bold text-emerald-400 flex items-center gap-1.5">
                    <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" />
                    Ativo ({oauthConfig.tokenValidityDays} dias)
                  </span>
                </div>
                <div className="flex items-center justify-between text-slate-300">
                  <span className="text-slate-400">Contas/Páginas Vinculadas:</span>
                  <span className="font-bold text-white">{selectedCount} selecionada(s)</span>
                </div>
                <div className="flex items-center justify-between text-slate-300">
        ...
        <div className="px-5 py-3 bg-slate-950/80 border-t border-slate-800/80 flex items-center justify-between text-[11px] text-slate-400">
          <div className="flex items-center gap-1.5">
            ...
            <span>Isolamento seguro de tokens por organização</span>
          </div>
